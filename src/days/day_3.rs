	...
	for i in 0..mask.len() {
		for j in 0..mask[i].len() {
			if mask[i][j] && grid[i][j].is_digit(10) {
				...
				while (0..mask[i].len() as i32).contains(&start) && grid[i][start as usize].is_digit(10) {
					start -= 1;
				}
				start += 1;
				let mut end = j;
				...
				let result: u32 = grid[i][start as usize..end as usize].iter().collect::<String>().parse().unwrap();
				count += result;
...
fn search(grid: &Vec<Vec<char>>, i: usize, j: i32) -> u32 {
	...
	grid[i][start as usize..end as usize].iter().collect::<String>().parse().unwrap()
}
	...
	for i in 0..grid.len() as i32 {
		for j in 0..grid[i as usize].len() as i32 {
			if grid[i as usize][j as usize] == '*' {
				...
				for k in [-1, 1] {
					if (0..grid.len() as i32).contains(&(i + k)) {
						if grid[(i + k) as usize][j as usize].is_digit(10) {
							...
							if (0..grid[(i + k) as usize].len() as i32).contains(&(j - 1)) && grid[(i + k) as usize][(j - 1) as usize].is_digit(10) {
								adjacent += 1;
				...
				if (0..grid[i as usize].len() as i32).contains(&(j - 1)) && grid[i as usize][(j - 1) as usize].is_digit(10) {
					adjacent += 1;
